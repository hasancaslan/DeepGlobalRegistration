...
def pointcloud_to_spheres(pcd, voxel_size, color, sphere_size=0.6):
    ...
    s = o3d.geometry.TriangleMesh.create_sphere(radius=voxel_size *
                                                sphere_size)
...
def compute_overlap_ratio(pcd0, pcd1, trans, voxel_size):
    ...
    matching01 = get_matching_indices(pcd0_down, pcd1_down, trans, voxel_size,
                                      1)
    matching10 = get_matching_indices(pcd1_down, pcd0_down,
                                      np.linalg.inv(trans), voxel_size, 1)
...
def get_matching_indices(source, target, trans, search_voxel_size, K=None):
    ...
    for i, point in enumerate(source_copy.points):
        [_, idx, _] = pcd_tree.search_radius_vector_3d(point,
                                                       search_voxel_size)
        if K is not None:
            idx = idx[:K]
        for j in idx:
...
def evaluate_feature_3dmatch(pcd0,
                             pcd1,
                             feat0,
                             feat1,
                             trans_gth,
  ...
  """
    if len(pcd0.points) < len(pcd1.points):
        hit = valid_feat_ratio(pcd0, pcd1, feat0, feat1, trans_gth,
                               inlier_thresh)
    else:
        hit = valid_feat_ratio(pcd1, pcd0, feat1, feat0,
                               np.linalg.inv(trans_gth), inlier_thresh)
    return hit
